<par>
{{< figure
    src="Logo_OSC_Torino.png"
    alt="The logo of the Open Science Community Torino"
    class="logo"
    >}}

<style>
    cc-logo {
        width: 200;
        height: 200;
    }
</style>

# Welcome!

The Open Science Community Torino is a grassroots organization made up of researchers, technicians, data stewards and citizens that teaches, fosters and promotes Open Science practices in academia and beyond. We sport members from all four of the largest research performing organisations in and around Torino: the University of Turin, the Politecnico di Torino, the Istituto Italiano di Ricerca Metrologica and the Università del Piemonte Orientale, but also unaffiliated citizens and people working in private companies. We’re not officially sponsored by any institution.

We are interested in the implementation of Open Sciences in all contexts and forms: from reproducible research to FAIR Data Stewardship, from ethical and transparent research to citizen science, from Open Access to the reform of scholarly communication and evaluation.
If you live or work in and around Turin and want to connect with fellow researchers in or outside your institution or company to discuss Open Science and its implementation, the Open Science Community is the place to be!

This homepage is in English to improve accessibility, but all of our content is mostly written or presented in **Italian**.

## Who is the OSCT for?

If you are a researcher or are interested in research, and are looking to be more collaborative, transparent, efficient and have greater outreach, the Open Science Community is for you! You don’t necessarily need to live in or around Torino to connect: if you’re as excited for Open Science as us, join today!

To create a safe, welcome space we ask that all of our members follow the [Community Guidelines](pages/code_of_conduct).

## What does the OSCT do?

The Open Science Community Torino aims to foster the adoption of Open, Reproducible and Transparent science practices in researchers that work in and around Torino. To do this, it focuses on four cornerstones: Organise the OS movement, Support researchers interested in Open Science, Connect them with each other to share methods and best practices and Teach newcomers of Open Science in its methods and philosophy.

If you find any collegues already part of the OSCT, send them an email to say hi and begin a discussion! If you don’t (and even if you do!), join our shared spaces to talk about Open Science, how to solve its problems and how best to put it in action.

You can read more about these ideas in the OSCT [Mission Statement](pages/mission).
If you're interested on how the OSCT works, check out the Handbook at [handbook.osc.to](https://handbook.osc.to/) to learn more!

## Join the Open Science Community Torino

If you have questions or concerns, feel free to email osct@unito.it to get in touch. If you are interested to join as a member of the Open Science Community Torino, check out the ["Join US" page](pages/join_us).

## Meet the Community Coordinators


<div style="display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1rem;">
  <img src="luca.png" alt="Luca" style="width: 120px; height: 120px; object-fit: cover; border-radius: 50%; flex-shrink: 0;">
  <div>
    <h3>Luca Visentin</h3>
    <p>Hello, I’m Luca! I’m deeply interested in Open Science and the ways in which we can do science better. My main focus is Data Stewardship and how research groups can manage their data openly and efficiently. I'm also interested in the phylosophy of Open Science and of data. I work at the Politecnico di Milano as a Data Steward, although I live in Turin.</p>
  </div>
</div>

<div style="display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1rem; flex-direction: row-reverse;">
  <img src="Ilaria.png" alt="Ilaria" style="width: 120px; height: 120px; object-fit: cover; border-radius: 50%; flex-shrink: 0;">
  <div>
    <h3>Ilaria Stura</h3>
    <p>Hi, I’m Ilaria, a statistics and mathematical models researcher in Medicine. I am enthusiastic about Open Science, and I’m interested in data quality and sharing, particularly in Medicine.</p>
  </div>
</div>

<div style="display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1rem;">
  <img src="evdokia.png" alt="Evdokia" style="width: 120px; height: 120px; object-fit: cover; border-radius: 50%; flex-shrink: 0;">
  <div>
    <h3>Evdokia Tema</h3>
    <p>Hi everyone! I am Evdokia and I am working at the Earth’s Science Department of the University of Torino. My main research focuses on understanding the variation of the Earth’s magnetic field in the past and exploring its applications in geology, volcanology, stratigraphy, archaeology, and cultural heritage. I strongly believe in Open Science and deeply engage the transparency, reproducibility and sharing of research results and knowledge. Let’s rise together to the challenge of restoring science to its social role and contribute, brick by brick, to fostering the FAIR principles in scientific research!</p>
  </div>
</div>

<div style="display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1rem; flex-direction: row-reverse;">
  <img src="Giglia.png" alt="Elena" style="width: 120px; height: 120px; object-fit: cover; border-radius: 50%; flex-shrink: 0;">
  <div>
    <h3>Elena Giglia</h3>
    <p>Hi, I’m Elena Giglia, I’m a passionate advocate for Open Science and FAIR data management. I have been lecturing and writing on these topics for years, enjoying a lot and meeting wonderful people. I’m involved in the European network of Open Science and FAIR and I’m a partner in several EU funded projects on Open research. Officially, I’m Head of the Open Science Unit at the University of Turin.</p>
  </div>
</div>

<div style="display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1rem;">
  <img src="mauro.png" alt="Mauro" style="width: 120px; height: 120px; object-fit: cover; border-radius: 50%; flex-shrink: 0;">
  <div>
    <h3>Mauro Paschetta</h3>
    <p>Hello, I’m Mauro! I work at the Polytechnic University of Turin as a domain expert in Open Science. I support and train researchers on research data management, with a focus on FAIR principles and Open Science practices.
    I have a PhD in Evolutionary Biology and Biodiversity Conservation and I worked as a project manager at the Department of Oncology of the University of Torino, where I was involved in research support and in the management of European projects.
    Over time, I’ve developed a strong interest in Open Science, data stewardship, and FAIR data, and I’ve followed several training courses in these areas. I’m currently part of the coordination group of the Italian Data Stewards Community (CIDS), a member of AISA (Associazione Italiana Scienza Aperta), and involved in the National Chapter of CoARA.</p>
  </div>
</div>

<div style="display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1rem; flex-direction: row-reverse;">
  <img src="mikaela.jpg" alt="Bob" style="width: 120px; height: 120px; object-fit: cover; border-radius: 50%; flex-shrink: 0;">
  <div>
    <h3>Mikaela Kuepferling</h3>
    <p>Hi, I am Michaela, a researcher at the National Institute of Metrological Research (INRiM) in Turin, where I work with the Magnetism, Materials, and Spintronics group. I currently guide the Open Science group at INRiM, which has existed since 2020. I believe the current methods for evaluating science quality and impact are flawed and limit research freedom. In my view, Open Science provides a vital pathway to overcoming these constraints.</p>
  </div>
</div>


